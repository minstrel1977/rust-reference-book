...
每个宏都有一个名称和一条或多条*规则*。每条规则都有两部分：一个*匹配器(matcher)*，描述它匹配的句法；一个*转换器(transcriber_)*，描述成功匹配后的将执行的替代调用句法。匹配器和转换器都必须由定界符包围。宏可以扩展到表达式、语句、数据项（包括 trait、impl 和外来数据项）、类型或模式。

## Transcribing
## 转换

当宏被调用时，宏扩展器(macro expander)按名称查找宏调用，并依次尝试其每条宏规则。宏会根据第一个成功的匹配进行转换；即使转换结果导致错误，也不会尝试进行后续匹配。在匹配时，不执行预判；如果编译器不能明确地确定如何一个标记码一个标记码地解析宏调用，则会报错。在下面的示例中，编译器不会提前查看标识符，以查看后跟的标记码是否为 `)`，尽管这将允许它明确地解析调用：
...
在匹配器和转换器中，标记码 `$` 用于从宏引擎中调用特殊行为(下文[元变量][Metavariables]和[重复段][Repetitions]中有详述)。不属于此类调用的标记码将按字面意义进行匹配和转换，除了一个例外。这个例外情况是匹配器的外层定界符将匹配任何一对定界符。因此，比如匹配器 `(())` 将匹配 `{()}`，而 `{{}}` 不行。字符 `$` 不能按字面意义匹配或转换。

当将匹配的代码片段转发给另一个声明宏时，第二个宏中的匹配器看到的将是此代码片段的不透明抽象句法树(opaque AST)。第二个宏不能使用字面量标记码来匹配匹配器中的代码片段，只能使用相同类型的代码片段指示符(fragment specifier)。片段类型 `ident`、`lifetime`、和 `tt` 是几个例外，它们*可以*通过字面量标记码进行匹配。下面示例展示了这一限制：
...
以下示例展示了在匹配 `tt` 类型的代码片段之后生成的标记码如何能够再次直接匹配：
...
在匹配器中，`$`*名称*`:`*代码片段指示符* 匹配符合指定句法类型的 Rust 代码片段，并将其绑定到元变量 `$`*名称*上。有效的代码片段指示符是：

  * `item`: [_数据项_][_Item_]
  * `block`: [_块表达式_][_BlockExpression_]
  * `stmt`: 句尾没有分号的[_语句_][_Statement_]（需要分号的数据项语句除外）
  * `pat`: [_模式_][_Pattern_]
...
因为代码片段类型已在匹配器中指定了，则在转换器中，元变量只简单地用 `$`*名称* 这种形式来指代就行了。元变量最终将被替换为跟它们匹配上的句法元素。元变量关键字 `$crate` 可以用来指代当前的 crate（请参阅后面的[卫生性][Hygiene]）。元变量可以被多次转换，也可以完全不转换。
...
- `*` — 表示重复任意次数。
- `+` — 表示至少重复一次。
- `?` — 表示一个可选代码片段，可以出现零次或一次。


因为 `?` 表示最多出现一次，所以它不能与分隔符一起使用。

重复的代码片段匹配并转换为指定数量的代码片段，由分隔符分隔。元变量与它们对应的代码片段的每次重复段相匹配。例如，上面示例中的 `$( $i:ident ),*` 将 `$i` 去匹配列表中的所有标识符。
...
2.  转换器中的每个重复段必须至少包含一个元变量，以便确定扩展多少次。如果在同一个重复段中出现多个元变量，则它们必须绑定到相同数量的代码片段上。例如，`( $( $i:ident ),* ; $( $j:ident ),* ) =>( $( ($i,$j) ),*` 里，绑定到 `$j` 的代码片段的数量必须与绑定到 `$i` 上的相同。这意味着用 `(a, b, c; d, e, f`) 调用这个宏是合法的，并且可扩展到 `((a,d), (b,e), (c,f))`，但是 `(a, b, c; d, e)` 是非法的，因为前后绑定的数量不同。此要求适用于嵌套的重复段的每一层。

## 作用域、导出以及导入
<!-- 第一次检查到这里 -->
由于历史原因，声明宏的作用域并不完全像数据项那样工作。宏有两种形式的作用域：文本作用域和基于路径的作用域。文本作用域基于代码在源文件中出现的顺序，甚至是跨多个文件出现的顺序，并且是默认的作用域。下面将进一步解释这个。基于路径的作用域与数据项作用域的工作方式完全相同。宏的作用域、导出和导入主要由属性控制。
