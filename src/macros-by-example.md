...
`macro_rules` 允许用户以声明方式定义句法扩展。我们称这种扩展形式为“声明宏（macros by example）”或简称“宏”。
...
当将当前匹配的匹配段转发给另一个声明宏时，第二个宏中的匹配器看到的将是此匹配段的不透明抽象句法树(opaque AST)。第二个宏不能使用字面量标记码来匹配匹配器中的匹配段，只能使用相同类型的匹配段类型指示符(fragment specifier)。片段类型 `ident`、`lifetime`、和 `tt` 是几个例外，它们*可以*通过字面量标记码进行匹配。下面示例展示了这一限制：
