...
*路径*是一个或多个由命名空间<span class="parenthetical">限定符(`::`)</span>*逻辑*分隔的路径段(path segments)组成的序列。如果路径仅由一个路径段组成，则它引用局部控制域(control scope)内的[数据项][item]或[变量][variable]。如果路径包含多个路径段，则总是引用数据项。
...
简单路径用于[可见性][visibility]标记、[属性][attributes]、[宏][macros]和 [`use`] 数据项。示例：
```rust
...
完全限定性路径可以用来为 [trait实现][trait implementations]消除路径歧义，也可以用来指定[规范路径](#canonical-paths)。用在指定具体类型时，它支持使用如下所示的类型指定句法：
...
类型路径用于类型定义、trait约束、类型参数约束，以及限定性路径。
...
以 `::` 开头的路径被认为是全局路径，其中的路径首段从当前 crate 根位置开始解析。路径中的每个标识符都必须解析为一个数据项。

> **版本差异**: 2015 版中，crate 根包含多种不同的数据项，包括：外部 crate、默认 crate（如 `std` 和 `core`），以及 crate 顶层的数据项（包括各种 `use` 导入数据项）。
>
...
$crate 仅用在[宏转换器][macro transcribers]中，且仅能用作路径首段，没有前置 `::`。`$crate` 将被扩展为从定义宏的 crate 的顶层访问该 crate 中的各数据项的路径，而不用去考虑宏调用发生时所在的现场 crate。
...
定义在模块或者实现中的数据项都有一个*规范路径*，该路径对应于其在其 crate 中定义的位置。该路径之外所有指向这些数据项的路径都是别名（路径）。规范路径被定义为附加在代表数据项自身定义路径段之前的一个*路径前缀*。

尽管实现中所定义的数据项有规范路径，但[实现][Implementations]本身和 [use声明][use declarations]没有规范路径。块表达式中定义的数据项没有规范路径。在没有规范路径的模块中定义的数据项也没有规范路径。在实现中定义的关联项(associated items)引用没有规范路径的数据项 - 例如，实现类型(implementing type)、被实现的 trait、类型参数或类型参数上的约束 - 都没有规范路径。

对于模块来说，模块的路径前缀就是该模块的规范路径。对于裸实现(bare implementations)来说，它下面数据项的路径前缀是它实现的数据项的规范路径，语义格式上要使用<span class="parenthetical">尖括号（`<>`）</span>括起来。对于 [trait实现][trait implementations]来说，它内部的数据项的路径前缀是被实现的数据项的规范路径后面，先跟一个 `as`，再跟 trait 本身的规范路径，然后整个使用<span class="parenthetical">尖括号（`<>`）</span>括起来。
