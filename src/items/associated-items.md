...
每个关联数据项都有两种形式：（包含实际实现的）定义和（为定义声明签名的）声明。[^译者备注]
...
参数列表中的第一个参数为 self类型(`self`)的参数的关联函数被称为*方法*，方法可以使用[方法调用操作符][method call operator](`.`)来调用，例如 `x.foo()`，也可以使用常用的函数调用形式进行调用。

如果 self类型(`self`)的参数被指定了，它就通过以下文法（其中 `'lt` 表示生存期参数）来把此指定的参数限制解析成此文法中的一个类型：
...
此文法中的 `Self`终结符(terminal)表示解析为实现类型的类型。这种解析包括解析上下文中的类型别名 `Self`、其他类型别名、或使用投射解析把（调用者的类型中的）关联类型解析为实现类型。
