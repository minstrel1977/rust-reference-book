...
本书是 Rust 编程语言的主要参考手册，本书提供了3类资料：
  ...
  - 一些章节非正式地介绍了该语言的内存模型、并发模型、运行时服务、联接模型，以及调试工具。
...
类似地，本书通常不能作为记录 rustc 或者 Cargo 细节的工具书。rustc 有自己专门的书[rustc book]，Cargo 也有一本书[cargo book]，该书中包含了 [Cargo参考资料]。在本书中，也包含一些诸如[联接][linkage]的章节，介绍了 rustc 是如何工作的。
...
本书不会假定您是按顺序阅读本书。本书的每一章一般都可以独立阅读，但会交叉联接到其他章节，以了解它们所引用的内容，但不进行讨论。
...
第二是提高您对该语言某一方面的认知。在这种情况下，只要浏览目录，直到你看到你想了解的内容，然后点击开始阅读。阅读中如果某个联接看起来很有趣，那就点击并阅读该部分内容。
...
* 定义术语的语句中，术语写为*斜体*。当术语在其定义章节之外使用时，通常会有一个联接指向术语定义的章节。
