...
>commit: 7b1240b2451be16ccfc9bd695e6ebb95f80b11c6 \
...
例如：模式 `'m'..='p'` 只匹配 `'m'`, `'n'`, `'o'` 和 `'p'` 这4个字符。 `1..` 可以匹配 9，或者 9001，或者 9007199254740991（如果此值的内存宽度合适），但是不能匹配到 0，也不能匹配有符号整数的负值。区间模式中的界值可以是字面量，也可以是指向常量的路径。
