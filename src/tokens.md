...
* *空白符转义*是 `U+006E` (`n`)、`U+0072` (`r`) 或者 `U+0074` (`t`) 之一，依次分别表示 Unicode 值 `U+000A`（LF），`U+000D`（CR），或者 `U+0009`（HT）。
...
*字节字面量*是单个 ASCII 字符(码值在 `U+0000` 到 `U+007F` 区间内)或一个*转义符*前接字符 `U+0062`（`b`）和 `U+0027`（单引号 `'`），后接字符 `U+0027`。如果字符 `U+0027` 出现在字面量中，它必须由前置字符 `U+005C`（`\`）转义。字节字面量等价于一个 `u8` 无符号 8-bit 整型*数字字面量*。
...
非原生*字节串字面量*是 ASCII 字符和转义字符组成的字符序列，形式是以字符 `U+0062`（`b`）和 `U+0022`（双引号 `"`）开头，以字符 `U+0022` 结尾。如果字面量中包含字符 `U+0022`，则必须由前置的 `U+005C`（`\`）_转义_。此外，字节串字面量也可以是*原生字节串字面量*(下面有其定义)。长度为 `n` 的字节串字面量类型为 `&'static [u8; n]`。
